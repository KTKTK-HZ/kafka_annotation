...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    // 发送消息的线程，sender对象会在该线程中运行，这是一个后台守护进程
    ...
    KafkaProducer(ProducerConfig config,
        ...
        try {
            ...
            /**
             * 在客户端请求服务端时，可能因为网络或服务端异常造成请求超时。这时请求失败会重试，
             * 但是如果重试的频率过高又可能造成服务端网络拥堵。因此必须等一段时间再请求，默认100ms。*/
            long retryBackoffMs = config.getLong(ProducerConfig.RETRY_BACKOFF_MS_CONFIG);

            //设置keySerializer和valueSerializer
            ...
            this.maxRequestSize = config.getInt(ProducerConfig.MAX_REQUEST_SIZE_CONFIG); // 设置消息的最大的长度，默认1M，生产环境可以提高到10M
            this.totalMemorySize = config.getLong(ProducerConfig.BUFFER_MEMORY_CONFIG);  // 设置发送消息的缓冲区的大小，默认32M
            this.compressionType = CompressionType.forName(config.getString(ProducerConfig.COMPRESSION_TYPE_CONFIG)); // 消息压缩类型

            this.maxBlockTimeMs = config.getLong(ProducerConfig.MAX_BLOCK_MS_CONFIG); // 设置等待元数据更新的最大时间，默认1分钟
            ...
            //producer中有两个线程，一个main，一个sender
            this.sender = newSender(logContext, kafkaClient, this.metadata);
            String ioThreadName = NETWORK_THREAD_PREFIX + " | " + clientId;
            //daemon=true，以守护线程的方式拉起，用 ioThread 线程来封装 Sender 线程类
    ...
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        // 将消息，拦截器，回调函数封装成一个AppendCallbacks对象
        ...
        try {
            ...
            // ClusterAndWaitTime是一个封装的类，用来存储Cluster和waitTime信息
    ...
    private ClusterAndWaitTime waitOnMetadata(String topic, Integer partition, long nowMs, long maxWaitMs) throws InterruptedException {
        ...
        // 如果元数据topics中没有该topic，则将其加入topics中，并将更新标志设为true，否则什么都不做
        metadata.add(topic, nowMs);
        // 如果该topic已经存在于元数据中，则返回该topic的总分区数，否则返回null
        Integer partitionsCount = cluster.partitionCountForTopic(topic);
        // Return cached metadata if we have it, and if the record's partition is either undefined
        // or within the known partition range
        // 如果元数据存在并在partition没有指定或者partition值小于总分区数，则直接返回
