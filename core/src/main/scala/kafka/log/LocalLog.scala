...
class LocalLog(@volatile private var _dir: File, // 日志所在的文件夹路径
   ...
   */
  def read(startOffset: Long, // 从log对象的哪个位移值开始读取消息
           maxLength: Int, // 能读取的最大的字节数
           minOneMessage: Boolean, // 如果为true，即使消息大于maxLength，也要返回一条消息
           maxOffsetMetadata: LogOffsetMetadata, // 要获取的最大偏移量的元数据
           includeAbortedTxns: Boolean /*如果为True，则包含终止事务*/): FetchDataInfo = {
