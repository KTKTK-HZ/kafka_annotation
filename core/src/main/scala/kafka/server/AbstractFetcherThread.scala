 ...
 * 它是一个抽象类，实现的功能是从Broker获取多个分区的消息数据，至于获取之后如何对这些数据进行处理，则交由子类来实现
 */
abstract class AbstractFetcherThread(name: String, // 线程名称
  ...
  protected def processPartitionData(topicPartition: TopicPartition, // 读取的哪个分区的数据
                                     fetchOffset: Long, // 读取到的最新offset
                                     partitionData: FetchData /*读取到的分区消息数据*/): Option[LogAppendInfo] // 写入已读取消息数据前的元数据

  // OffsetTruncationState：封装了一个位移值和一个截断完成与否的布尔值状态
  ...
  /**
   * 作为leader的分区可能会随时发生变化。每当有新Leader产生时，Follower副本就必须主动执行截断操作，
   * 将自己的本地日志裁剪成与Leader一模一样的消息序列，甚至，Leader副本本身也需要执行截断操作，
   * 将LEO调整到分区高水位处
   * */
  ...
  private[server] def truncateToHighWatermark(partitions: Set[TopicPartition]): Unit = inLock(partitionMapLock) {
    ...
    for (tp <- partitions) {
      ...
      if (partitionState != null) {
        ...
        // 创建OffsetTruncationState结构记录结果，OffsetTruncationState是case class
...
case class PartitionFetchState(topicId: Option[Uuid],
  ...
  // 截断中，表明分区副本正在执行截断操作（比如该副本刚刚成为Follower副本），分区处于截断中状态的条件：副本处于Truncating状态且未被推迟执行
  def isTruncating: Boolean = state == Truncating && !isDelayed
  // 被推迟，表明副本获取线程获取数据时出现错误，需要等待一段时间后重试，分区被推迟获取数据的条件：存在未过期的延迟任务。
  //该状态一般用于判断是否需要推迟获取对应分区的消息
