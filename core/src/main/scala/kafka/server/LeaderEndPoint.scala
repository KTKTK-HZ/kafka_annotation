...
trait LeaderEndPoint {
   ...
   * 为指定分区构建对应的FetchRequest.Builder对象，而该对象是构建FetchRequest的核心组件。
   * Kafka中任何类型的消息读取，都是通过给指定Broker发送FetchRequest请求来完成的。
   * 输入一组要读取的分区列表，分区是否可读取取决于PartitionFetchState中的状态
   */
